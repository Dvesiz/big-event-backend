...
`big-eventBackend` 是一个为“大事件”应用提供后端服务的项目。它旨在为前端应用提供稳定、高效的 RESTful API 接口，支持用户管理、事件（文章）管理、分类管理等核心功能。本项目的目标是构建一个可扩展、易维护的后端服务，为各类事件发布、管理平台提供坚实的数据支撑。
